---
title: "CABLAB R. Workshops -- Introduction to R"
date: "11/21/2023"
author: 
...
This workshop will provide an introduction into R! R is a popular programming language that many researchers use for organizing data, visualizing data, and carrying out statistical analyses.

By the end of the workshop series, my hope is that you will feel comfortable enough to work independently in R!


![**A fair warning**](/Users/tuh20985/Desktop/CABLAB R Workshops/R memes/psychology statistics.jpeg)

## Outline

| Topic | Description |
| Pre-Workshop: Set Up & Prerequisites: Downloading R Software | R: https://cran.r-project.org/ RStudio: https://posit.co/download/rstudio-desktop/
| Week 1: Intro to R | Learning how to navigate the software: R, RStudio, Console, and creating scripts in R Markdown
| Week 2: Installing and Loading Packages: Swirl() | Continue orienting to the software environment using the "swirl" package
...
| Week 9: Analyzing Data w/ Categorical Independent Variables | Conducting statistical analyses (t-tests and anovas) with categorical predictors
| Week 10: Analyzing Data w/ Continuous Independent Variables | Conducting statistical analyses (bivariate linear regressions, multiple linear regressions, and linear mixed effect regressions) with continuous and categorical predictors
| Week 11: Visualizing data: Intro to ggplot | Learn how to create ggplot visualizations and customize plots
| Final Project? Conclusion? TBD | TBD 


# --- Pre-Workshop: Set Up & Prerequisites: Downloading R Software ---
...
3. Download the 'CABLAB R Workshop' folder from the CABLAB R Workshop Github. This is the folder containing the all the files we will be working with for the purposes of this workshop.
...
![](/Users/tuh20985/Desktop/CABLAB R Workshops/R Markdown insert code.png){width=35%}
![](/Users/tuh20985/Desktop/CABLAB R Workshops/R Markdown addition.png){width=20%}

1. If we want to leave notes, we don’t have to “comment it out”. We can just write long-winded narration that can help others understand why we coded what we coded and what that code does. 

Below are examples of R accepting code when it's commented out and not accepting code when it's not commented out.

![](/Users/tuh20985/Desktop/CABLAB R Workshops/R Script hashtag.png){width=40%}
![](/Users/tuh20985/Desktop/CABLAB R Workshops/R Script no hashtag.png){width=40%}

2. That’s because a typical script will interpret any text as a command, unless the text is otherwise marked by a hashtag (#). An R markdown script only interprets things as code when we tell it to, and we tell it what is code by creating a chunk. Chunks are marked by three backticks (```) followed by a {r} and, on another line, three more backticks.

![](/Users/tuh20985/Desktop/CABLAB R Workshops/R Markdown blah blah blah.png){width=35%}

A typical script can’t make sense of this, though. We need to use R markdown scripts to do it. You might be thinking, though, that manually denoting code from non-code seems like extra work, and it is a little bit, but it can also be a lot more convenient because …

3. The output of any given chunk will appear in the R Studio Console Window. By output, we just mean the product, sum, or status of whatever calculation or item you are asking R to compute and show you.
...
## --- Week 2: Installing and Loading Packages: swirl() ---
...
![](/Users/tuh20985/Desktop/CABLAB R Workshops/R Packages.png){width=50%}
...
# --- Week 2: Installing and Loading Packages: swirl() --- Exercise ---
```{r Exercise 1, code="'\n\n\n\n'", results=F}
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/R/solutions/week2_exercise.r)



## --- Week 2 Assignment: Install and Load "swirl" library and complete "R Programming: The basics of programming in R" modules 1 (Basic Building Blocks) - 7 (Matrices and Data Frame) ---
...
Let's give this a try and we can talk through any problems people ran into during our next workshop.

![](/Users/tuh20985/Desktop/CABLAB R Workshops/R swirl 1.png){width=70%}
![](/Users/tuh20985/Desktop/CABLAB R Workshops/R swirl 2.png){width=70%}
...
Here's how to check out where where your current working directory is using the getwd() command.
...
![](/Users/tuh20985/Desktop/CABLAB R Workshops/R tab.png){width=40%}
...
# For Windows
Path <- "C:/Users/tuh20985/Desktop/CABLAB R Workshops/"

# For Mac
Path <- "/Users/tuh20985/Desktop/CABLAB R Workshops/"
...
There were also 3 experimental conditions: Contorl, Share, and Test.
...
Test condition: Participanst were instructed to tour the huaunted house segment in anticipation of being tested on their knowledge of the haunted house segment afterwards.
...
![](/Users/tuh20985/Desktop/CABLAB R Workshops/fright night study design.png){width=70%}

![](/Users/tuh20985/Desktop/CABLAB R Workshops/fright night study design conditions.png){width=60%}
...
![](/Users/tuh20985/Desktop/CABLAB R Workshops/recency discrimination example.png){width=70%}
![](/Users/tuh20985/Desktop/CABLAB R Workshops/recency discrimination full.png){width=70%}
...
![](/Users/tuh20985/Desktop/CABLAB R Workshops/LIWC example.png){width=70%}
...
![](/Users/tuh20985/Desktop/CABLAB R Workshops/df environment.png){width=70%} 
...
Condition column -- You'll also see that there are 3 conditions: Baseline, Share, and Test.

Participants completed the haunted house segment in the Baseline, Share, or Test condition.

Baseline condition: "Tour the haunted house as you normally would"

Share condition: "We're going to ask you to post about this haunted house segment immediately after you finish it. Please tour the haunted house like you plan on posting about it afterwards"

Test condition: "We're going to test you on your knowledge for the haunted house segment immediately after you finish it. Please tour the haunted house like you plan on being tested on it afterwards"
...
## --- Week 3: Working Directories --- Exercise ---

As an exercise, let's read in the df_wide CSV file and run the head() and View() functions on the dataset
...
## --- Week 3 Assignment: Working Directories -- No assignment for Week 3 ---
...
## Understanding how to reference rows and columns in data frames
...
Here, we are telling R to subset all rows where Section is equal to Infirmary OR Asylum. As you can tell, leveraging the OR (|) or ANd (&) operators within the subset() function can be especially flexible.
...
## --- Week 4: Subsetting in R --- Exercise ---
...
Do this twice: Once using bracket notation and another time using the subset() function.
...
## --- Week 4 Assignment: Subsetting in R ---
...
df_clean <- subset(df, select=c(PID, Section, Stage, Threat, Recall, TOAccuracy))
...
Let's use an ifelse statement to create a new column that represents whether a Section was a High Threat or "Low Threat section

The structure for ifelse() statements is as follows: dataframe$name_of_new_column <- if the value in the Section column has a cell that = "Infirmary", we will insert a value of "Low" in the new Threat column for that cell, else, if the value in the Section column is anything other than "Infirmary", insert a value of "High" to represent high threat
...
We can also do this with more than one conditions using the OR (i.e., |) operator or the AND (i.e., &) operator.
...
What if we want to organize our columns in a certain order.
...
#Approach 2: Re-organize one specific column in a data frame
df_reorganized <- df %>% relocate(Threat, .after = Group)
df_reorganized <- df %>% relocate(Threat, .before = Recall)
...
## --- Week 5: If Else statements --- Exercise ---

Use the ifelse() function to create a new column called Year, where, if the Section was equal to Infirmary or Ghostly Grounds, assign a value of "2019", else, assign a value of "2021".
...
#Make a new column called RecallLength where word count less than or equal to 40 is "Short", word count in between 40 and 60 is "Medium", and word count greater than oe equal to 60 is "Long"
...
## --- Week 6: Intro to For Loops in R ---
...
## --- Week 6: Intro to For Loops in R --- Exercise ---
...
Next, let's map the for loop onto the dataset we've been using! I'll also try to convert the structure of the ifelse() function into the structure of a for loop "if else" statement in case that context would be helpful.
```{r}
...
## --- Week 6 Assignment: Intro to for loops in R ---

#https://statisticsglobe.com/if-else-statement-in-r/

1) Let's create a vector called x4 that contains the following values: "A", "B", "A", "D", "A", "F"
1) Create a for loop that iterates across all values in x3. If the value == A, print "This value represents A", else, print "This value represents, B, D, or F"
...
df_one_sub <- subset(df_analyses, PID == "1001")
...
# --- Week 7: Pivoting data from wide to long and long to wide --- Exercise ---
...
```{r Week 7 Exercise - hidden, eval=T, include=F}
...
#Read in the df_wide.csv file
df_wide_assignment <- read.csv(file = "df_wide_assignment.csv")
...
Learning how to merge multiple data frames is an integral part of data cleaning.

Let's create a data frame called "df_merge" that has the following columns from the df data frame: "PID", "Section", "Stage", "Threat", "Group", "Condition", and "TOAccuracy"

Next, let's subset and creating two new data frames from the df_new data frame: 1 data frame that only has assessments from the Immediate study visit, and a second data frame that has all the assessments from the 1 week Delay data frame.
...
Here's some more info about the all.x and all.y arguments in the merge() function
all.x=TRUE is logical; which means if TRUE, then all extra rows that exist in one data frame but not the other, will be added to the data frame. In other words, each row in X data frame that has no matching row in Y data frame will be added to the new data frame.
Same philosophy applies for all.y=TRUE
...
# --- Week 8: Merging data frames --- Exercise ---

1) Subset two data frames from the df_new data frame: 1 data frame called "low_df" that only contains rows where the Temporal Memory Accuracy score is less than .5. A second data frame that only contains rows where the Temporal Memory Accuracy score is greater than .5.
...
4) Merge the two data frames back together!
...
Some of the more common data types include: character, factor, numeric, integer.
...
## Analyzing Data w/ Categorical Independent Variables
...
# --- Week 9: Analyzing Data w/ Categorical Independent Variables --- Exercise ---
...
```{r Week 9 Exercise, code="'\n\n\n\n'", results=F}


  
```

```{r Week 9 Exercise - hidden, eval=T, include=F}
...
## Analyzing Data w/ Continuous Independent Variables
...
# --- Week 10: Analyzing Data w/ Continuous Independent Variables --- Exercise ---
...
## --- Week 10 R Assignment: Analyzing Data w/ Continuous Independent Variables ---
...
# --- Week 11: Visualizing data: Intro to ggplot --- Exercise ---

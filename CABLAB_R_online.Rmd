...
require(knitr)
opts_knit$set(root.dir = '/Users/tuh20985/Desktop/CABLAB R Workshops/')
...
| **Pre-Workshop**: Downloading R Software | **R**: https://cran.r-project.org/ **RStudio**: https://posit.co/download/rstudio-desktop/
...
![](images/R Script hashtag.png){width=40%}


and rejecting code notes when it's not commented out!

![](images/R Script no hashtag.png){width=40%}
...
**Condition** column -- You'll also see that there are 3 conditions: Baseline, Share, and Test. Participants completed the haunted house segment in the Baseline, Share, or Test condition.
...
2) Create a for loop that iterates across all values in x3. If the value == A, print "This value represents A", else, print "This value represents, B, D, or F"
...
As we can see, this data frame is pretty messy! No column names, columns are not in order, there are missing values, and the data is in wide format! Let's work on cleaning this up and start by adding some column names.
...
df_raw <- subset(df_raw, select=c(PID, Group, Condition, Aslyum.WordCount, GhostlyGrounds.WordCount, DevilsDen.WordCount, Infirmary.WordCount, Aslyum_TOAccuracy, GhostlyGrounds_TOAccuracy, DevilsDen_TOAccuracy, Infirmary_TOAccuracy))
...
6) Re-organize the position of the columns so that it 
...
df_raw <- read.csv(file = "frightnight_raw_assignment.csv") #Load in the fright night raw csv file
